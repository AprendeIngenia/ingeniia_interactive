...
// --- Pequeños componentes para mantener el código limpio (sin cambios) ---
...
// --- Componente de la Tarjeta de Video (sin cambios) ---
...
        {tag && <span className="px-2 py-1 text-xs font-bold text-background bg-neon-cyan rounded-full">{tag}</span>}
      </div>
...
const VideosSection = () => {
  ...
  // ¡Aquí está la magia! Toda la lógica de fetching se resume en una línea.
  const { data: videos, isLoading, error } = useFetchData(contentService.getVideosByTopic, 'mlp');
