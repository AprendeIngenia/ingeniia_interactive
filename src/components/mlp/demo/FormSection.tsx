...
const InputField = ({ icon: Icon, label, ...props }: any) => (
  <div className="relative">
    <label className="block text-sm font-medium text-muted-foreground mb-1 ml-1">
      {label}
    </label>
    <div className="relative">
...
const FormSection: React.FC<FormSectionProps> = ({ onPredicted }) => {
  ...
  const handleChange: React.ChangeEventHandler<HTMLInputElement | HTMLSelectElement> = (e) => {
    ...
    // Soporta tanto number como range para mantener número
    const v = type === "number" || type === "range" ? Number(value) : (value as any);
  ...
  const handleSubmit: React.FormEventHandler = async (e) => {
    ...
    // Puerta de autenticación: si no hay sesión, abrir modal y evitar submit
    if (!isAuthenticated) {
      toast({
        title: "Necesitas iniciar sesión",
        description:
          "Crea tu cuenta o inicia sesión para usar el demo del MLP.",
      });
      openAuthModal();
      return;
    }
      ...
      toast({
        title:
          output.prediction === "good"
            ...
            : "Necesitamos reforzar tu perfil",
        description:
          output.prediction === "good"
  ...
  return (
    <section id="application-form" className="py-24 bg-[#0A0A0A] border-y border-white/5">
      ...
      >
        ...
        {/* Aviso de acceso si no está autenticado */}
        {!isAuthenticated && (
          <div className="max-w-4xl mx-auto mb-6 p-4 rounded-xl border border-yellow-500/30 bg-yellow-500/5 text-sm text-yellow-200">
            Para usar el demo necesitas una cuenta.{" "}
            <button
        ...
        <form
          ...
          className="max-w-4xl mx-auto p-8 bg-card/50 border border-white/10 rounded-2xl shadow-2xl shadow-glow-purple"
        >
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-muted-foreground mb-2 ml-1">
                ...
                <span className="font-bold text-neon-cyan">
                  {jobLabels[Number(formData.Job) ?? 0]}
                </span>
              </label>
              <input
                type="range"
